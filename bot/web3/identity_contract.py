...
async def register_identity_onchain(owner_private_key: str) -> int | None:
    ...
    try:
        ...
        if not _is_insufficient_funds(e):
            log.error("ERC-8004 register() (delegated attempt) failed: %s", e)
            return None
        log.warning(
            "Delegated gas path rejected by RPC (no active relayer). "
    ...
    try:
        ...
        log.error("ERC-8004 register() (owner-paid) failed: %s", e)
        return None
